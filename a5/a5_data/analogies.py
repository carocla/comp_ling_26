...
def read_turney_analogies(embeddings, path = '/projects/e31408/data/a5/SAT-package-V3.txt'):   
# def read_turney_analogies(embeddings, path = 'SAT-package-V3.txt'):  
    questions = []
    letters = ['a','b','c','d','e']
...
def answer_by_analogy(embeddings, question, choices):
    ...
    max_score = -1
    a,b=question
    a=embeddings.__getitem__(a)
    b=embeddings.__getitem__(b)
    for index, choice in enumerate(choices):
        aa,bb=choice
        aa=embeddings.__getitem__(aa)
        bb=embeddings.__getitem__(bb)
        score = embeddings.cosine_similarity(a - b + bb, aa)
        if score > max_score:
            max_score=score
            max_index=index
    return max_index
...
def answer_by_parallelism(embeddings, question, choices):
    ...
    for index, choice in enumerate(choices):
        ...
        aa=embeddings.__getitem__(aa)
        bb=embeddings.__getitem__(bb)
        score = embeddings.cosine_similarity(a - b, aa - bb)
        if score > max_score:
            max_score=score
            max_index=index
    return max_index
...
def evaluate(embeddings, dataset, method = answer_by_analogy):
    ...
    for dic in dataset:
        ...
        choices = dic['choices']
        answer = dic['answer']
        estimated_answer = method(embeddings, question, choices)
